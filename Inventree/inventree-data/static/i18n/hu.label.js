...
function printLabels(options) {
    ...
    if (options.items.length > 1) {
        header_html += `
            ...
            ${options.items.length} ${options.plural_name} kiválasztva
            </div>
        `;
    }
    ...
    const updatePrintingOptions = (formOptions) => {
        ...
        if (Object.keys(printingOptions).length > 0) {
            formOptions.fields = {
                ...formOptions.fields,
                divider: { type: "candy", html: `<hr/><h5>Nyomtatási beállítások</h5>` },
                ...printingOptions,
            };
        }
    ...
    const printingFormOptions = {
        title: options.items.length === 1 ? `Címke nyomtatása` : `Címkék nyomtatása`,
        submitText: `Nyomtatás`,
        method: "POST",
        disableSuccessMessage: true,
        header_html,
        fields: {
            _label_template: {
                label: `Címke sablon választás`,
                type: "choice",
                localOnly: true,

...
function printLabels(options) {
        ...
        success: function (response) {
            if (response.length == 0) {
                showAlertDialog(
                    ...
                    'Nenhum modelo de etiqueta corresponde aos itens selecionados',
                );
                return;
            }
    ...
    if (options.items.length > 1) {
        header_html += `
            ...
            ${options.items.length} ${options.plural_name} selecionado
            </div>
        `;
    }
    ...
    const updatePrintingOptions = (formOptions) => {
        ...
        if (Object.keys(printingOptions).length > 0) {
            formOptions.fields = {
                ...formOptions.fields,
                divider: { type: "candy", html: `<hr/><h5>Opções de Impressão</h5>` },
                ...printingOptions,
            };
        }
    ...
    const printingFormOptions = {
        title: options.items.length === 1 ? `Imprimir Etiqueta` : `Imprimir etiquetas`,
        submitText: `Imprimir`,
        method: "POST",
        ...
        fields: {
            _label_template: {
                label: `Selecione o modelo de etiqueta`,
                type: "choice",
                localOnly: true,

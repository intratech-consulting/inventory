...
function selectReport(reports, items, options={}) {
    ...
    if (items.length > 0) {
        ...
        ${items.length} kiválasztott tételek
        </div>`;
    }
    ...
    modalSetTitle(modal, 'Teszt riport sablon kiválasztása');
    ...
    inventreeGet(options.url, params, {
        success: function(response) {
            if (response.length == 0) {
                showAlertDialog(
                    'Nem található riport',
                    'Nem található riport sablon a kiválasztott tételekhez',
                );
                return;
            }

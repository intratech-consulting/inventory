...
function printLabels(options) {
    ...
    if (options.items.length > 1) {
        header_html += `
            ...
            ${options.items.length} ${options.plural_name} выбрано
            </div>
        `;
    }
    ...
    const updatePrintingOptions = (formOptions) => {
        ...
        if (Object.keys(printingOptions).length > 0) {
            formOptions.fields = {
                ...formOptions.fields,
                divider: { type: "candy", html: `<hr/><h5>Параметры печати</h5>` },
                ...printingOptions,
            };
        }
    ...
    const printingFormOptions = {
        title: options.items.length === 1 ? `Печать Этикетки` : `Печать этикеток`,
        submitText: `Печать`,
        method: "POST",
